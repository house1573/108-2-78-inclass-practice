...
在R裡`{...}`（大括號, brace）：
  ...
  * 依照上一個特性，`function(...){....}`的body若只有一行指令可省略`{...}`改寫成
    `function(...) ...`, 只是要小心body … 要接在`function(...)`之後不可換行。
...
```
  ...
  * 若最後一個被執行的指令會產生print在螢幕的效果，則可以用`{...}->`或`<-{...}`將這
    print的值存出來。
...
```
  ...
  * 依照上一個特性，`function(...){....}`的body若最後一個「被執行」的指令是會印在螢幕
    的值，如該值要當回傳值可以不用`return(...)`就可以產生值回傳效果。
...
一家三口，
  ...
  * 早上(06:00-09:00)： 媽媽讀《經濟學人》，爸爸讀《WSJ》，提醒小明“檢查書包東西都有
    帶嗎？”

  * 晚上(17:00-19:00)： 媽媽準備全家晚餐點foodpanda，爸爸瀏覽tripadvisor
    (https://www.tripadvisor.com/)規劃週末全家旅行，提醒小明“作業寫好了嗎？”

  * 晚上(23:30-次日淩晨1點)： 媽媽聆聽Spotify Jazz Classics
