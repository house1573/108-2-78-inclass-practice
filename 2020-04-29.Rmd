...
list_victimAges_female$`年齡層` <- levels_new
```
情境
```{r}
sum(list_victimAges_female$`數目`, na.rm=T)
...
total_na <- sum(pick_na) #logic TRUE=1 FALSE=0
print(total_na)
```
...
找出問題中包含“肺炎”字眼的問題。
```{r}
pick_QuestionLung <- stringr::str_detect(CDC_chatbox$Question, "肺炎")
CDC_chatbox$Question[pick_QuestionLung]
```
