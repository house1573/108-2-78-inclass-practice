...
函數使用時若出現output value assigned動作時（即`pass_fail_i <-`），相當於執行了函數`body pass_fail_i <-{...}`，依switch小節的`{...}`處理原則：
...
```
  ...
  * Body: `{get_courses(academicYears[[.]], allGrades[[.]], courseStructure)}`,
    以formula（另一種function形式）包裝。
...
幾乎每一個iteration都可以用functional, for, while寫一次，but …
  ...
  * 不過若沒有追求時間效率的需求，還是以符合直覺的寫法優先，日後維護也比較能進入狀
    況。
